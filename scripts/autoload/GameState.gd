...
func get_data(path: String) -> Variant:
	if not _loaded:
		DebugManager.log_warn(name, "No state data is loaded, data may be invalid (get): %s" % path)
...
func set_data(path: String, value: Variant) -> Error:
	...
	if not _loaded:
		DebugManager.log_warn(name, "No state data is loaded, data may be invalid (set): %s" % path)
		return FAILED
	return OK
